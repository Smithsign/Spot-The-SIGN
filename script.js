...
function renderSign() {
  ...
  signWidth = ctx.measureText(text).width;
  const fontSize = parseInt(ctx.font, 10);
  
  // Generate valid positions
  signX = Math.random() * (canvas.width - signWidth);
  signY = fontSize + (Math.random() * (canvas.height - fontSize * 2));
  
  // Draw the sign
  ctx.fillStyle = "red";
  ctx.fillText(text, signX, signY);
